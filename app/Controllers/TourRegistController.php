    ...
    public function list_room_price()
    {
        ...
        if ($s_date && $e_date) {
            $sql = "SELECT a.*, b.bed_idx, b.bed_type, b.bed_seq FROM tbl_room_price a
									...
									WHERE a.product_idx = '" . $product_idx . "' $search1 AND a.bed_idx != '' AND a.bed_idx != null AND a.goods_date BETWEEN '$s_date' AND '$e_date' ";
        } else {
            $sql = "SELECT a.*, b.bed_idx, b.bed_type, b.bed_seq FROM tbl_room_price a
			                        LEFT JOIN tbl_room_beds b ON a.bed_idx = b.bed_idx 
			                        WHERE goods_date >= '". $today ."' AND a.bed_idx != ''  AND a.bed_idx != null AND product_idx = '" . $product_idx . "' $search1 ";
        }
        $result = $this->connect->query($sql);
        $nTotalCount = $result->getNumRows();
