...
//        }
	...
	public function getHotelProductList($where, $perPage, $page, $orderBy)
	{
		$offset = ($page - 1) * $perPage;
		$baht_thai = homeSetInfo()['baht_thai'] ?? 1.0;

		$builder = $this->db->table('tbl_product_mst p')
			->select('p.*, 
				(p.product_price * '.$baht_thai.') AS product_price_won,
				GROUP_CONCAT(DISTINCT CONCAT_WS("::", r.room_name, r.rooms_idx) ORDER BY r.rooms_idx ASC SEPARATOR "|") AS room_info,
				GROUP_CONCAT(DISTINCT i.ufile ORDER BY i.onum ASC, i.i_idx ASC SEPARATOR "|") AS images')
			->join('tbl_hotel_rooms r', 'p.product_idx = r.goods_code AND r.is_view_promotion = "Y" AND r.room_name != ""', 'left')
			->join('tbl_product_img i', 'p.product_idx = i.product_idx AND i.ufile != ""', 'left')
			->where('p.product_status', 'sale')
			->groupBy('p.product_idx');

		// 필터링
		if (!empty($where['product_code_1'])) $builder->where('p.product_code_1', $where['product_code_1']);
		if (!empty($where['product_code_2'])) $builder->where('p.product_code_2', $where['product_code_2']);
		if (!empty($where['product_code_3'])) $builder->where('p.product_code_3', $where['product_code_3']);
		if (!empty($where['keyword'])) $builder->like('p.product_name', $where['keyword']);

		// 가격 필터는 환율 반영 후 PHP에서 수행
		// 정렬
		foreach ($orderBy as $col => $dir) $builder->orderBy($col, $dir);
