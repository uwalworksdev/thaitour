...
// 기본 필터
foreach (['product_code_1','product_code_2','product_code_3'] as $key) {
    if (!empty($where[$key])) {
        $builder->where($key, $where[$key]);
    }
}
...
// 배열 검색어
if (!empty($where['arr_search_txt'])) {
    $this->addMultiWordSearch($builder, ['product_name','product_name_en','keyword'], $where['arr_search_txt']);
}
