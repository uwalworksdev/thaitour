                ...
                <div id="container" class="main" style="margin-top: 0;margin-left: 0;width: 100%;">
                    ...
                    <div class="bod_list_cont main_cont">
                        ...
                        <li>
                            ...
                            <ul class="bod_list">
                            <?php
                                $sql_s = "  select * from tbl_bbs_list  where code = 'b2b_notice' and status = '' order by notice_yn desc, r_date desc limit 0,3 ";
                                $result_s = $db->query($sql_s)->getResultArray();
                                foreach($result_s as $row_s)
                                {
                            ?>
                            <li>
                                <a href="/AdmMaster/_bbs/board_write?scategory=&search_mode=&search_word=&code=b2b_notice&bbs_idx=<?=$row_s['bbs_idx']?>&pg=1"><span class="sb_ttl"><?= $row_s['subject'] ?> </span><em class="date"><?= substr($row_s['r_date'], 0, 10) ?></em></a>
                            </li>
                            <?php 
                                }
                            ?>
                        ...
                        <li>
                            ...
                            <ul class="bod_list">
                            <?php
                                $sql_a = " SELECT * FROM tbl_bbs
                                                    WHERE r_code = 'faq' AND r_status = 'Y' 
                                                    ORDER BY r_date desc limit 0, 3 ";
                                $result_a = $db->query($sql_a)->getResultArray();
                                foreach ($result_a as $row_a)
                                {
                            ?>
                                <li>
                                    <a href="/AdmMaster/_bbs/view?cmd=view&r_idx=<?=$row_a['r_idx']?>&page=1&r_code=faq"><span class="sb_ttl"><?=$row_a['r_title']?></span><em class="date"><?=date("Y.m.d", strtotime($row_a['r_date']))?></em></a>
                                </li>
                            <?php
                                }
                            ?>
                        ...
                        <li>
                            ...
                            <ul class="bod_list">
                            <?php
                                $sql_b = " SELECT A.*, COUNT(B.r_idx) AS cmt_cnt
                                                    FROM tbl_travel_qna A
                                                    LEFT JOIN tbl_bbs_cmt B ON A.idx = B.r_idx AND B.r_code = 'qna'
                                                    GROUP BY A.idx
                                                    ORDER BY A.idx desc limit 0, 3 ";
                                $result_b = $db->query($sql_b)->getResultArray();
                                foreach ($result_b as $row_b)
                                {
                            ?>
                            <li>
                                <a href="/AdmMaster/_qna/detail.php?idx=<?=$row_b['idx']?>"><span class="sb_ttl"><?= $row_b['title'] ?></span><em class="date"><?=date("Y.m.d", strtotime($row_b['r_date']))?></em></a
                                >
                            </li>
                            <?php
                                }
