...
<div id="container"> <span id="print_this"><!-- 인쇄영역 시작 //-->
	...
	<header id="headerContainer">
		<div class="inner">
			...
			<div class="menus">
				<ul >
					<li><a href="/AdmMaster/_review/list.php" class="btn btn-default"><span class="glyphicon glyphicon-th-list"></span><span class="txt">리스트</span></a></li>
					<?php if ($row) { ?>
				...
				<table cellpadding="0" cellspacing="0" summary="" id="table_form_online_quote" class="listTable mem_detail">
					...
					<tbody>
						<tr>
							...
							<td>
								<select name="status">
									<option value="Y" <?php if ($status == "Y") {echo "selected"; } ?>>승인</option>
									<!-- <option value="Y" <?php if ($status == "Y") {echo "selected"; } ?>>상담완료</option> -->
									<option value="N" <?php if ($status == "N") {echo "selected"; } ?>>미승인</option>
								</select>
							</td>
							<?php if ($row) {?>
							<th>게시날짜</th>
							<td><input type="text" value="<?= date('Y-m-d H:i:s', strtotime($r_date)) ?>" name="r_date" id="r_date"></td>
							<?php } ?>
						</tr>
						<tr>
							<th>베스트</th>
								<td>
									<input type="checkbox" name="is_best" id="is_best" <?=($is_best == "Y" ? "checked" : "")?>>
								</td>
							<th>메인노출</th>
							<td>
								<input type="checkbox" name="display" id="display" <?=($display == "Y" ? "checked" : "")?>>
							</td>
						</tr>
						<tr>
							<th>여행자 성함</th>
						...
						<tr>
							...
							<td colspan="3">
								<div class="depart flex__c gap-1">
										...
										?>
										<select name="travel_type_2" id="travel_type_2" style="margin-left: 5px;<?=(!$travel_type ? "display: none;" : "")?>">
											<option value="">선택</option>
											<?php
										...
										?>
										<select name="travel_type_3" id="travel_type_3" style="margin-left: 5px;<?=(!$travel_type_2 ? "display: none;" : "")?>">
											<option value="">선택</option>
											<?php
											foreach($result as $row) {
												?>
										...
										<select name="product_idx" id="products" style="<?=(!$travel_type_3 ? "display: none;" : "")?>">
											<option value="">선택</option>
											<?php
