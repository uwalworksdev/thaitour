...
if ($search_gubun == "is_best") {
    $strSql = $strSql . "AND is_best = 'Y' "; 
}

if ($search_gubun == "display") {
    $strSql = $strSql . "AND display = 'Y' "; 
}



$total_sql = " select *, CASE WHEN is_best = 'Y' THEN 1 ELSE 0 END AS is_best_in_number
						from tbl_travel_review where 1=1 $strSql ";
		...
		<div id="contents">
			<form name="search" id="search">
				...
				<select id="search_gubun" name="search_gubun" class="input_select" style="width:112px">
						<option value="" <? if ($search_gubun == "") {
													echo "selected";
												} ?>>전체</option>
						<option value="is_best" <? if ($search_gubun == "is_best") {
														echo "selected";
													} ?>>베스트</option>
						<option value="display" <? if ($search_gubun == "display") {
														echo "selected";
													} ?>>메인노출</option>
					</select>

					<select id="" name="search_category" class="input_select" style="width:112px">
						<option value="title" <? if ($search_category == "title") {
													echo "selected";
												} ?>>제목</option>
						<option value="user_name" <? if ($search_category == "user_name") {
														echo "selected";
													} ?>>국문이름</option>
						<option value="user_email" <? if ($search_category == "user_email") {
														echo "selected";
													} ?>>이메일</option>
					</select>
			...
			<div class="listWrap">
					...
					<div class="listBottom">
						<table cellpadding="0" cellspacing="0" summary="" class="listTable">
							...
							<tbody>
								...
								$sql    = $total_sql . " order by onum desc, r_date desc limit $nFrom, $g_list_rows ";
								//echo $sql;
								...
								?>
									<tr style="height:50px;">
										...
										<td>
											<input type="checkbox" <?if($row['is_best'] == "Y") echo "checked";?> class="input_check">
											<input hidden type="text" name="is_best[]" value= <?=$row['is_best'] == "Y" ?  "Y" :"N" ?> >
									</td>
										<!-- <td>
											<input type="checkbox" <?if($row['display'] == "Y") echo "checked";?> class="input_check">
				...
				<? echo ipageListing($pg, $nPage, $g_list_rows, $_SERVER[PHP_SELF] . "?&search_gubun=$search_gubun$s_status&search_category=$search_category&s_txt=$s_txt&pg=") ?>
