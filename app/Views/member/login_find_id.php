...
<main id="container" class="sub find member pt100">
	<div class="inner_620">
		...
		<div class="login_tab">
			<button type="button" class="on" onclick="location.href='./login_find_id'">아이디 찾기</button>
			<button type="button" onclick="location.href='./login_find_pw'">비밀번호 찾기</button>
		</div>
		...
		<section class="login_cont">
			...
			<div class="login_box email_box">
				<div class="input-group">
					<div class="input-row">
						<input type="text" name="name2" id="name2" class="bs-input" placeholder="이름을 입력하세요.">
					</div>
					<div class="input-row">
						<div class=" email_row">
			...
			<!-- //  이메일로 찾기  // -->
			<div class="btn-wrap"><button type="button" class="btn btn-lg find_btn btn-point"
					onclick="confirm_it();">확인</button></div>
		</section>
	</div>
...
<script>
	function cert_it_1() {
		...
		$.ajax({
			url: "cert_id_send_sms",
			type: "POST",
			data: "mobile=" + mobile + "&user_name=" + $("#user_name").val(),
	...
	function cert_it_2() {
		...
		const user_email = $("#email_1").val() + "@" + $("#email_2").val();
		$.ajax({
			url: "cert_id_send_email",
			type: "POST",
			data: "user_email=" + user_email + "&user_name=" + $("#name2").val(),
			error: function (request, status, error) {
				//통신 에러 발생시 처리
				alert("code : " + request.status + "\r\nmessage : " + request.reponseText);
			}
